...
export default function OffPlanPropertiesPage() {
    ...
    const [isFiltersOpen, setIsFiltersOpen] = useState(false); // Default to closed as per user request
    ...
    const [filters, setFilters] = useState<PropertyFilterValues>({
        ...
        minArea: 0,
        maxArea: 50000,
        status: '', // Empty string means show all properties
        reference: '',
        location: '',
        permitNumber: '',
    });
        ...
        <div className="flex flex-col h-screen bg-white p-8 overflow-hidden">
            ...
            <div className="flex-shrink-0 flex items-center justify-between mb-6">
                <h1 className="text-[24px] font-semibold text-[#1A1A1A]">
                    All Properties <span className="text-[#8F9BB3] font-medium ml-1">({properties.length})</span>
                </h1>
            ...
            {/* Helper Grid for Layout */}
            <div className="flex flex-1 min-h-0 gap-6">
                {/* Left Sidebar - Filters - Conditionally Rendered */}
                {isFiltersOpen && (
                    <div className="w-[340px] flex-shrink-0 bg-white border-r border-[#EDF1F7] pr-6 h-full overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                        <PropertyFilters
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    {/* Search & Filters Bar */}
                    <div className="flex-shrink-0 flex items-center gap-3 bg-white py-4 mb-4 rounded-xl">
                        {/* Search */}
                        ...
                        {isLoading ? (
                            ...
                            <VirtuosoGrid
                                ...
                                itemContent={(index: number) => (
                                    <OffPlanPropertyCard property={properties[index]} />
                                )}
                            />
                        )}
                    </div>
